...
Elm.TagInput = Elm.TagInput || {};
Elm.TagInput.make = function (_elm) {
   "use strict";
   _elm.TagInput = _elm.TagInput || {};
   if (_elm.TagInput.values) return _elm.TagInput.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var _op = {};
   var onEdit = F2(function (address,value) {
      return A3($Html$Events.on,
      "input",
      $Html$Events.targetValue,
      function (str) {
         return A2($Signal.message,address,value(str));
      });
   });
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "Add": var text = model.inputText;
           var id = $List.length(model.tags) + 1;
           var newTag = {id: id,text: text};
           return _U.update(model,
           {tags: A2($List._op["::"],newTag,model.tags),inputText: ""});
         case "Edit": return _U.update(model,{inputText: _p0._0});
         default: return _U.update(model,
           {tags: A2($List.filter,
           function (tag) {
              return !_U.eq(tag.id,_p0._0);
           },
           model.tags)});}
   });
   var model = {tags: _U.list([]),inputText: ""};
   var Remove = function (a) {    return {ctor: "Remove",_0: a};};
   var onRemove = F2(function (address,id) {
      return A2($Html$Events.onClick,address,Remove(id));
   });
   var tagToHtml = F2(function (address,tag) {
      return A2($Html.div,
      _U.list([A2(onRemove,address,tag.id)]),
      _U.list([$Html.text(tag.text)]));
   });
   var tagList = F2(function (address,tags) {
      return A2($List.map,tagToHtml(address),tags);
   });
   ...
   var onEnter = function (address) {
      ...
      $Html$Events.keyCode,
      function (key) {
         return A2($Signal.message,address,isEnter(key));
      });
   };
   var tagInput = F2(function (address,inputText) {
      return A2($Html.input,
      _U.list([onEnter(address)
              ,A2(onEdit,address,Edit)
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([]),
      A2($List._op["::"],
      A2(tagInput,address,model.inputText),
      A2(tagList,address,model.tags)));
   });
   var main = $StartApp$Simple.start({model: model
                                     ,update: update
                                     ,view: view});
   var Model = F2(function (a,b) {
      return {tags: a,inputText: b};
   });
   var Tag = F2(function (a,b) {    return {id: a,text: b};});
   return _elm.TagInput.values = {_op: _op
                                 ,Tag: Tag
                                 ,Model: Model
                                 ,NoOp: NoOp
                                 ,Add: Add
